...
-- PostgreSQL schema for NEON (Neon Auth + Neon RLS)
-- ----------------------------------------------------------------------------
-- This is database/schema.sql ported off Supabase. Tables, columns,
-- constraints, indexes and functions are unchanged; only the identity plumbing
-- differs. One row per decision:
--
--   Supabase                         Neon
--   -------------------------------  -------------------------------------
...
--   trips.user_id         uuid       text
--   saved_places.user_id  uuid       text
--   role "anon"                      role "anonymous"
--   trigger on auth.users            trigger on neon_auth.users_sync
--
-- Why text: neon_auth.users_sync.id is TEXT (Stack Auth ids), so every column
-- holding a user id follows it. Every other uuid (trip id, day id, ...) is
-- unchanged. pg_session_jwt also ships an auth.uid() returning uuid, but
-- pairing text with text avoids a cast in all 12 policy predicates.
--
-- Two deliberate deviations from the original:
--
--   1. admin_overview is created WITH (security_invoker = true). The original
--      carried a comment claiming RLS still applied to it, but a Postgres view
--      runs as its OWNER unless that option is set -- so on the original the
--      view actually bypassed RLS. Setting it makes the comment true.
--
--   2. The signup hook fires on neon_auth.users_sync, which Neon populates by
--      SYNC rather than inside the signup transaction. A profile row therefore
--      appears shortly after signup, not atomically with it. Call
--      public.ensure_profile() from the first authenticated request when it
--      has to exist right now.
--
-- RLS binds a connection that authenticates as "authenticated" carrying a
-- Stack Auth JWT. neondb_owner OWNS these tables and, like any Postgres table
-- owner, BYPASSES row level security -- that is the server-side/service path.
--
-- Run order:  schema.neon.sql  ->  (optional) seed.sql
...
-- ----------------------------------------------------------------------------
-- WHY current_user_id() EXISTS INSTEAD OF CALLING auth.user_id() DIRECTLY
--
-- pg_session_jwt puts auth.user_id() in a schema owned by Neon's cloud_admin,
-- with acl {cloud_admin=UC/cloud_admin}. neondb_owner can call it through role
-- membership, but "authenticated" cannot, and neondb_owner cannot grant what it
-- does not hold with grant option -- GRANT USAGE ON SCHEMA auth silently does
-- nothing rather than failing.
--
-- A policy predicate written as "user_id = auth.user_id()" is evaluated as the
-- INVOKING role, so as "authenticated" it raises 42501 "permission denied for
-- schema auth" instead of filtering rows. The owns_trip()/owns_day() helpers are
-- SECURITY DEFINER and would have kept working -- which is what makes this easy
-- to miss: the nine child tables behave, while profiles, trips and saved_places
-- fail outright.
--
-- So every policy goes through this one SECURITY DEFINER wrapper, owned by
-- neondb_owner, which can reach the auth schema. search_path is pinned empty
-- and the call fully qualified, so the definer context cannot be hijacked.
-- ----------------------------------------------------------------------------
...
-- ============================================================================
-- 2. PROFILES  (1:1 with neon_auth.users_sync)
-- ============================================================================
...
comment on table public.profiles is 'Public profile mirror of neon_auth.users_sync. Never expose email/phone through a share link.';
...
-- ============================================================================
-- 15. API CACHE  (server-side only — never readable by end users)
-- ============================================================================
...
comment on table public.api_cache is
  'Server-side response cache. RLS is enabled with NO policies and NO grant to authenticated, so only the owner/service connection can read or write it.';
...
-- ============================================================================
-- 18. NEW-USER HOOK — create a profile row when Neon Auth syncs the user
-- Supabase fired this on auth.users inside the signup transaction. Neon syncs
-- users_sync out of band, so this runs a moment later. ensure_profile() is the
-- synchronous escape hatch for the first authenticated request.
-- ============================================================================
...
begin
  ...
  values (
    new.id,
    coalesce(new.email, ''),
    coalesce(new.name, new.raw_json ->> 'display_name'),
    new.raw_json ->> 'profile_image_url'
  )
...
drop trigger if exists on_auth_user_created on neon_auth.users_sync;
...
-- ============================================================================
-- 19. OWNERSHIP HELPERS — used by every child-table policy
-- ============================================================================
...
as $fn$
  select exists (
    ...
    where t.id = p_trip_id and t.user_id = public.current_user_id()
  );
$fn$;
...
as $fn$
  select exists (
    ...
    where d.id = p_day_id and t.user_id = public.current_user_id()
  );
$fn$;
...
set search_path = public
as $fn$
  select coalesce((select p.is_admin from public.profiles p where p.id = public.current_user_id()), false);
$fn$;
...
-- api_cache: RLS enabled, zero policies -> owner/service only. Do not add one.

-- ---- table grants ----------------------------------------------------------
-- RLS filters rows, but the role still needs the privilege. api_cache is
-- deliberately absent from this list.
grant select, insert, update, delete on
  public.profiles, public.trips, public.itinerary_days, public.itinerary_activities,
  public.routes, public.hotels, public.restaurants, public.sightseeing_places,
  public.expenses, public.packing_items, public.trip_checklists, public.saved_places,
  public.trip_shares
  to authenticated;
...
grant execute on function public.get_shared_trip(text) to anonymous, authenticated;
...
-- security_invoker = true so the view runs with the QUERYING user's rights and
-- RLS on the underlying tables still applies. Without it a Postgres view runs
-- as its owner and would bypass RLS entirely.
-- ============================================================================
